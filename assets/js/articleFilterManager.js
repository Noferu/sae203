  ...
  articles.forEach((article) => {
    if (article.image_url && article.title) {
      ...
      console.warn(
        ...
        article
      ); // Avertissement si les propriétés manquent
    }
  });
...
function processFilters(
  ...
  createCheckboxes(
    ...
    "category",
    preselectedCategoryId
  ); // Crée les cases à cocher pour les catégories
  createCheckboxes(
    "#subcategory-checkboxes",
    response.subcategories,
    "subcategory",
    preselectedSubcategoryId
  ); // Crée les cases à cocher pour les sous-catégories
  createCheckboxes(
    "#seller-checkboxes",
    response.sellers,
    "seller",
    preselectedSellerId
  ); // Crée les cases à cocher pour les vendeurs
  createCheckboxes(
    "#keyword-checkboxes",
    response.keywords,
    "keyword",
    preselectedKeywordId
  ); // Crée les cases à cocher pour les mots-clés
}
...
function generateArticleHtml(article) {
  ...
  let shortDescription =
    article.description.length > 100
      ? getShortDescription(article.description, 100)
...
function getShortDescription(description, maxLength) {
  ...
  if (
    description.charAt(maxLength) !== " " &&
    trimmedDescription.indexOf(" ") !== -1
  ) {
...
$(document).ready(function () {
  ...
  loadFilters(
    ...
    preselectedKeywordId,
    preselectedSellerId
  ); // Charge les filtres avec les présélections
